...
def call_bedrock_kb_retrieve(query, kb_id):
    ...
    try:
        ...
        print(f"[AdviceFn] KB retrieval error: {e}")
        return ""
...
def call_bedrock_claude(prompt, model_id):
    ...
    try:
        ...
        print(f"[AdviceFn] Bedrock Claude error: {e}")
        return f"Unable to generate advice at this time. Please contact your healthcare provider."
...
def lambda_handler(event, context):
    """
    Generates personalized health advice using Bedrock KB + Claude.
    Queues final advice messages to NotifyQueue.
    """
    processed = 0
    ...
    for record in event['Records']:
        ...
        print(f"[AdviceFn] Advice generated for {output['contact_uuid']}: {advice[:80]}...")
        processed += 1
